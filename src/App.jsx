...
// Liens Stripe pour chaque offre
const STRIPE_LINKS = {
...
const features = [
  {
    icon: Shield,
    title: "Affiches pro, prêtes à imprimer",
    text: "Mise en page nette (A4/A3), typographies lisibles, marges et grilles soignées.",
  },
  {
    icon: Sparkles,
    title: "IA pour texte & visuels",
    text: "Contenu rédigé avec GPT et illustrations générées avec style cohérent.",
  },
  {
    icon: CreditCard,
    title: "Commande en 2 minutes",
    text: "Formulaire + paiement Stripe. Livraison PDF HD par e-mail (POD en option).",
  },
];

const pricing = [
  {
    title: "Affiche unique",
    price: "49–149 €",
    perks: ["1 thème", "1 révision incluse", "PDF HD + aperçu web"],
  },
  {
    title: "Pack 5 affiches",
    price: "199–499 €",
    perks: ["5 thèmes", "3 révisions", "PDF HD + archives"],
  },
  {
    title: "Abonnement trimestriel",
    price: "99–199 €/mois",
    perks: ["2 affiches / mois", "priorité prod.", "suivi dédié"],
  },
];
...
export default function AffichesIALanding() {
  ...
  const handleCheckout = () => {
    if (!selectedOffer) {
      alert("Veuillez d'abord choisir une offre.");
      return;
    }
    setLoading(true);
    window.location.href = STRIPE_LINKS[selectedOffer];
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-violet-900 via-indigo-800 to-sky-700 text-white">
      ...
      <header className="mx-auto max-w-6xl px-6 pt-16 pb-8">
        ...
        >
          <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm">
            <Sparkles className="h-4 w-4" /> Affiches pédagogiques / métiers à
            la demande
          </div>
          <h1 className="mt-6 text-4xl font-bold leading-tight md:text-6xl">
            Créez des{" "}
            <span className="text-cyan-300">affiches personnalisées</span> en
            quelques minutes
          </h1>
          <p className="mt-4 max-w-3xl text-lg text-white/90">
            Sensibilisation sécurité, procédures métiers, supports éducatifs.
            Contenu rédigé + illustrations, mise en page automatisée, PDF HD
            prêt à imprimer. Option impression (POD).
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <a
              href="#pricing"
              className="rounded-2xl bg-cyan-400 px-5 py-3 text-black hover:bg-cyan-300"
            >
      ...
      <section className="mx-auto max-w-6xl px-6 py-8">
        <div className="grid gap-4 md:grid-cols-3">
          {features.map((f, i) => (
            <Card
              key={i}
              className="rounded-2xl bg-white/10 text-white backdrop-blur-sm"
            >
      ...
      <section className="mx-auto max-w-6xl px-6 py-8">
        <div className="rounded-3xl bg-white p-6 text-neutral-900">
          <div className="flex items-center gap-2 text-sm text-neutral-600">
            <ImageIcon className="h-4 w-4" /> Exemple d’affiche générée
            (cybersécurité au travail)
          </div>
          <p className="mt-2 text-neutral-700">
            Téléchargez l’aperçu et le PDF HD dans la conversation (“Affiche
            cybersécurité”). Vous pourrez les utiliser comme démonstrateur MVP.
          </p>
        </div>
      </section>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {pricing.map((p) => {
            ...
            return (
              <Card
                key={p.title}
                className={`rounded-2xl bg-white text-neutral-900 ${
                  selectedOffer === key ? "ring-4 ring-cyan-400" : ""
                }`}
              >
                <CardContent className="p-6">
      ...
      <section className="mx-auto max-w-6xl px-6 py-12" id="form">
        <div className="rounded-3xl bg-white p-6 text-neutral-900">
          ...
          <p className="mt-1 text-sm text-neutral-600">
            Donnez-nous les infos essentielles. Nous générons le contenu et la
            mise en page. Vous validez avant impression.
          </p>
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <Input
              label="E-mail de livraison"
              type="email"
              placeholder="vous@entreprise.fr"
            />
            <Input
              label="Société / École (optionnel)"
              placeholder="Nom de l’organisation"
            />
            <Input
              label="Thème de l’affiche"
              placeholder="p. ex. Cybersécurité au travail"
            />
            <Input label="Couleur principale (hex)" placeholder="#6C63FF" />
          </div>
          <div className="mt-4 grid gap-4">
            <TextArea
              label="Texte / règles à inclure (brefs points)"
              rows={5}
              placeholder="Listez 5–10 points clés…"
            />
            <label className="flex items-center justify-between rounded-xl border border-neutral-200 bg-neutral-50 p-4 text-sm text-neutral-700">
              <div className="flex items-center gap-2">
                <Upload className="h-4 w-4" /> Logo (PNG/SVG) – optionnel
              </div>
      ...
      <footer className="border-t border-white/10 bg-black/20 py-8">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-6 text-sm text-white/80">
          ...
          <div className="flex items-center gap-2">
            <Mail className="h-4 w-4" /> contact@syloria.com
          </div>
          <div className="flex items-center gap-2">
            <Palette className="h-4 w-4" /> Marque blanche sur demande
          </div>
        </div>
      </footer>
    </div>
